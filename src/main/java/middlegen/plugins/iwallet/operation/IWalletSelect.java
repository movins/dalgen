...
public class IWalletSelect extends IWalletOperation {
    ...
    public String getColumnType() {
         ...
         * (getTable().getColumn(item.getColumn()))).getJavaType();
         * }
         * } else {
    ...
    public String getMappedStatementSqlForPaging() {
        ...
        if (isHasSqlmap()) {
            pagingSql.append("SELECT * FROM (").append("SELECT T1.*, rownum linenum FROM (")
                    .append(getMappedStatementSqlNoAnnotation()).append(") T1 WHERE rownum &lt;= #")
                    .append(getEndRowName())
